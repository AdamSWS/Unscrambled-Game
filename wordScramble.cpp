...
public:
    ...
    void arrayInitialization(int area); //initializies arrays to have an index equal to that of the game's area
...
void Game::rotateRow(int RowColNum, int numRotate) { //function to rotate row
    for(int i = 0; i < abs(numRotate); i++){ //num times that the user wants row to rotate
        if(numRotate > 0){ //rotate left
            ...
            for(int j = 0; j < gameSize - 1; j++){ //index for every board pice in a row
...
void Game::setScrambleSize(){ //sets the number of times that random rows and columns of the board will rotate before user display
    ...
    if (numScramble < 1) { //checks if numSramble is less than 1
...
void Game::arrayInitialization(int area){ //initializes arrays from the max values an array can carry to the area size
    //Initialize arrays
    board[0] = board[area];
...
void Game::boardAssign(vector <string> file, int numSpaces){ //assigns values to each board index
    int count = 0; //interating value durring value assignments to prevent overlap
    while(numSpaces > 2){ //while there is greater than two spaces left on the board
        ...
        if(file.at(index).size() > numSpaces){ //if the size of the file's index value is greater than the number of spaces remaining
        ...
        else{ //when above ins't the case
            string sampleWord = file.at(index); //asign the files index value to a string
            ...
            if(numSpaces != 0){ //if there is no remaining spaces
                ...
                count++; //interate count
            }
        }
    }
    if(numSpaces > 0){ //for the remaining numSpace values
        for(int j = count; j < numSpaces; j++){ //interate through each value
...
void Game::Winners( int area){
    ...
    if(sample1 == sample2){ //if both above strings are the same
        ...
        cout << endl << "Current board words: ";
        for(int i = 0; i < area; i++){
            cout << complete[i]; //cout characters
        }
...
void completeGame() { //all game functions
    Game User;
    vector <string> file; //file Vector storing all of the words in the dictionary
